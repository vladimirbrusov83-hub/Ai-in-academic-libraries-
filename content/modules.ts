...
**ChatGPT (OpenAI)** is the most widely recognized AI tool globally and has the largest user community, which means the most community resources, tutorials, and library-specific examples available through professional channels. For example, the ACRL and ALA discussion boards have accumulated substantial practitioner experience with ChatGPT that is not yet available for newer tools. The free tier uses GPT-4o and is genuinely capable for most drafting and editing tasks, though it imposes rate limits that become noticeable with daily use. The paid tier (Plus, approximately $20/month) removes those limits and adds access to newer GPT-5 series models as well as OpenAI's o-series reasoning models — o1 and o3 — which are designed for complex multi-step problems rather than drafting. For library work, the standard GPT-4o or GPT-5 models handle most tasks well; the reasoning models are valuable for tasks involving complex decision trees or multi-step evaluation, such as assessing a vendor contract against a checklist of institutional requirements. For data privacy, inputs on the free tier may be used to improve OpenAI's models unless the user explicitly opts out in account settings — a step many users have not taken.

**Claude (Anthropic)** is particularly strong for tasks involving long documents and nuanced, sustained writing, and the Claude 4 model family — Opus 4, Sonnet 4, and Haiku 4 — has meaningfully expanded both capability and speed. Claude's context window is among the largest available, at 200,000 tokens, which means a librarian can paste an entire accreditation self-study document and ask Claude to identify gaps relative to a specific standard without hitting length limits that would affect other tools. Claude's Projects feature allows users to attach persistent documents — a collection development policy, an institutional style guide, a list of approved databases — that remain available across all conversations within the project. Such functionality is particularly useful for librarians who want consistent, institution-specific output across recurring tasks. Anthropic is generally regarded as more conservative in its approach to data use, and the Pro plan ($20/month) provides full access with clearer privacy terms than the free tier.

**Gemini (Google)** is most valuable for librarians and institutions whose primary workflow runs through Google Workspace. Gemini integrates directly into Google Docs, Gmail, Drive, and Slides, allowing it to work with documents in the places where they already live rather than requiring copy-paste workflows. The current Gemini 3.1 Pro model supports a one-million-token context window and handles text, images, audio, and video in a single conversation — which is particularly relevant for digital collections librarians working with mixed media. Gemini also has access to Google Search as a grounding mechanism, which reduces (but does not eliminate) hallucination on factual questions. For institutions that have signed Google Workspace enterprise agreements, the privacy terms may differ from consumer Gemini — this requires verification with institutional IT rather than assumption.
...
In library contexts, the hallucination types that carry the most professional risk are the ones that look most authoritative: invented citations with plausible journal names, volume numbers, and page ranges; statistics that sound reasonable but cannot be traced to a primary source; incorrect dates for events or policies; wrong author attributions; and legal or policy characterizations that are outdated or simply wrong. Such errors are not random — they tend to occur precisely where the model has learned that authoritative-sounding specificity is expected. A hallucinated citation does not announce itself; it is formatted correctly and plausible in content. Recognizing this pattern is the first step in the professional practice of verification that Module 05 covers in full.
...
In order to communicate the workflow's design and quality standards to administrators and colleagues, it is useful to document the workflow in a brief written protocol that specifies the tasks included, the review percentages applied, the quality thresholds that trigger workflow adjustment, and the audit schedule. Such documentation supports the professional accountability principle that human professionals are responsible for AI-assisted outputs — and provides the institutional record needed to evaluate the workflow's performance and defend its use if questions arise about record quality.`,
      ...
      practitionerNote:
        "At my institution, the task that made the strongest case for AI-assisted cataloging was not the most sophisticated application — it was date normalization. We had a legacy collection where birth and death dates in the 100 field appeared in four different formats across approximately eight thousand records, a known problem we had deferred for years because the manual correction time was prohibitive. I processed the records through Claude with a precise normalization rule and a set of examples, reviewed the output on a sample of two hundred records before committing to the full batch, and corrected the outliers where the rule had not applied cleanly. The entire project took two days of work rather than an estimated several weeks. The experience did not make me less careful about AI in cataloging — if anything, seeing how precisely the rules needed to be specified to produce clean output made me more attentive to what professional specification actually requires. In order to identify similar opportunities at your institution, I recommend auditing your catalog for the most common consistency problems and asking whether each one has a rule precise enough to be delegated to AI. If the rule can be written clearly, the task is a candidate.",
      summary: [
...
Professional AI tool subscriptions for individual librarians typically cost between $20 and $30 per user per month for the tiers that provide meaningful professional functionality — Claude Pro, ChatGPT Plus, and their equivalents. For a library team of five, a full-year subscription commitment is approximately $1,200-$1,800 per year. Such a figure should be presented alongside a time-savings estimate: if each team member saves thirty minutes per day on drafting and administrative tasks, the annual recovered capacity across five librarians is approximately 625 hours. At a mid-range librarian salary of $60,000 plus benefits, that recovered capacity has an institutional value that substantially exceeds the subscription cost. Such a comparison does not require the administration to accept the time-savings estimate as precise — it requires only that the estimate is reasonable and that the comparison makes the scale relationship between cost and potential return visible.
...
If the library drafts the policy rather than waiting for IT or legal to draft it, the library owns the framework. Such ownership has practical consequences: a policy written by library professionals will include provisions that reflect the FERPA obligations, the patron privacy commitments, and the professional disclosure norms that library practice requires. A policy written by IT or legal without library input may be technically sound but professionally insufficient — addressing data security without addressing the professional obligation to disclose AI use, for example, or treating library patron data as equivalent to general institutional data without recognizing the specific confidentiality protections that library records carry. Furthermore, a library that proactively brings a draft policy to administration signals institutional maturity: it is demonstrating that the library is not asking for permission to experiment without a plan, but proposing a responsible framework that the institution can adopt with confidence.`,
      ...
      practitionerNote:
        "The conversation I was most nervous about was not with my direct supervisor but with the dean of academic affairs, who had asked for a brief on what the library was doing about AI. I had fifteen minutes and a genuine uncertainty about whether she was likely to be supportive or skeptical. What I brought was simple: the Pulse of the Library data showing where academic libraries were globally, the 7% U.S. optimism figure alongside the peer country comparisons, a one-page description of the pilot we wanted to run, and a draft policy outline. I did not know in advance which of those four things would land. What actually shifted the conversation was the policy outline. She had been waiting for someone to bring her a governance framework for AI in the library rather than a request to use AI in the library. The framing that worked was not 'we want to adopt AI' — it was 'we want to build the institutional infrastructure to do this responsibly, and here is what that looks like.' I have thought about that distinction often since. The ask that succeeds is the one that resolves the concern the administrator is already carrying, not the one that introduces a new priority.",
      summary: [
        ...
        "A well-designed pilot has four required characteristics: bounded scope (one tool, one use case, defined period), a pre-specified measurable outcome, low institutional stakes to start, and a defined review point that generates a recommendation rather than extending ambiguity.",
